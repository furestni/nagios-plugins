...
my %tier;
# = (
#	'Storage_IX_Performance_Perf+'  => 1,
#	'Storage_IX_Performance_Perf'   => 1,
#	'Storage_IX_Performance_Medium' => 1,
#	'Storage_IX_Performance_Eco'    => 1,
#	'Storage_BIE_Performance_Perf+' => 1,
#	'Storage_BIE_Performance_Perf'  => 1,
#	'Storage_BIE_Performance_Medium'=> 1,
#	'Storage_BIE_Performance_Eco'   => 1,
#	);
...
#0de0a28a-b5998eb7:ix_eco_z1_p1_c1:nl.b-ix-dc.stor.int:/ifs/eco/virt/cloud/ix_eco_z1_p1_c1:fc52e8fcdcb6337bba47c1191a854264:Storage_IX_Performance_Eco
#
        while (<IF>) {
                chomp;
                my ($id,$nickname,$x,$other,$t) = split(';');
...
print_log (sprintf ("OK - %s - %s | %s", Opts::get_option('datacenter'), Opts::get_option('tier'), $icinga));
	...
	$time_stamps =~ s/Z//g;
      ...
      foreach (@$values) {
		if (($_->id->counterId == 144) || ($_->id->counterId == 145)) {
			...
			my $txt1 = Opts::get_option('tier');
			 #push (@{$tier{$t}{'id'}}, $id);
			 #print "---Tier---\n";
			 #print Dumper @{$tier{Opts::get_option('tier')}{'id'}} ;
			 #print "now: ",$_->id->instance,"\n";
			 my $idnow =  $_->id->instance;
			 #print Dumper grep (m/$idnow/, @{$tier{Opts::get_option('tier')}{'id'}} );
			 next unless (grep (m/$idnow/, @{$tier{Opts::get_option('tier')}{'id'}} ));
			 #print "cont: $idnow\n";
