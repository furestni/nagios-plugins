...
if ($o_check_type eq "ciscoNEW") {
	...
	my $index;
	my @temp_oid;
	my ($num_sensors,$num_sensors_ok,$num_thresholds,$num_thresholds_ok,$num_thresholds_warning) = (0,0,0,0,0);
							...
							if (defined($CiscoThreshold_value1)){
								...
								if ($CiscoThreshold_severity2 ne "noSuchInstance") {
									...
									if (($CiscoValue < $CiscoThreshold_value1) || ($CiscoThreshold_severity2 == 10)){
										...
										$result_t = $session->get_request( Varbindlist => \@temp_oid);	
										my $mydesc = $$result_t{$cisco_ios_xe_physicaldescr.".".$index};
										$mydesc =~ s/\,//g;
										$mydesc =~ s/\s+/_/g;
										$mydesc .= "_" . $index;
										$perfdata.= " " . $mydesc . "=" . $CiscoValue . ";" . ($$resultat_threshold_value{$cisco_ios_xe_threshold_value.".".$index.".1"}) . ";" . ($$resultat_threshold_value{$cisco_ios_xe_threshold_value.".".$index.".2"}-5) . ";0;" . $$resultat_threshold_value{$cisco_ios_xe_threshold_value.".".$index.".2"};
										$num_thresholds_ok++;
									} 
									elsif (($CiscoValue >= $CiscoThreshold_value1) && ($CiscoValue < $$resultat_threshold_value{$cisco_ios_xe_threshold_value.".".$index.".2"}-5)){
										@temp_oid=($cisco_ios_xe_physicaldescr.".".$index);
										$result_t = $session->get_request( Varbindlist => \@temp_oid);	
										my $mydesc = $$result_t{$cisco_ios_xe_physicaldescr.".".$index};
										$mydesc =~ s/\,//g;
	...
	if ($num_sensors!=0) {
		...
		if (($num_thresholds_warning > 0) && (($num_thresholds - $num_thresholds_warning - $num_thresholds_ok) == 0)) {
                  $output.= $num_thresholds_warning . " sensors reported WARNING (".$num_thresholds_ok." thresholds reported OK)";
                }
		else {
	...
	if ($final_status == 2) {
		print $output," : CRITICAL |" . $perfdata . "\n";
		exit $ERRORS{"CRITICAL"};
	}

	if ($final_status == 1) {
		print $output," : WARNING |" . $perfdata . "\n";
		exit $ERRORS{"WARNING"};
	}

	print $output," : OK |" . $perfdata . "\n";
	exit $ERRORS{"OK"};
}
