...
#	printf "%3d	  %-32s   %s\n", $id, $interface->{$id}{'ifAlias'}, $txtOperStatus[$interface->{$id}{'ifOperStatus'}] if ($interface->{$id}{'ifAdminStatus'} == 1);
	if (($interface->{$id}{'ifAdminStatus'} == 1) && ($interface->{$id}{'ifOperStatus'} != 1)) {
		printf "%3d	  %-32s   %s\n", $id, $interface->{$id}{'ifAlias'}, $txtOperStatus[$interface->{$id}{'ifOperStatus'}] if ($opt{'verbose'});
		push(@msg, sprintf("%s=%s", $interface->{$id}{'ifAlias'}, $txtOperStatus[$interface->{$id}{'ifOperStatus'}]));
		$rc=2;
	}
}
...
check port status of Cumulus Switches
