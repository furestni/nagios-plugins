...
my $cisco_nexus_used          = "1.3.6.1.4.1.9.9.109.1.1.1.1.12.1"; # Used memory
my $cisco_nexus_free          = "1.3.6.1.4.1.9.9.109.1.1.1.1.13.1"; # Free memory
...
my $foundry_total         = "1.3.6.1.4.1.1991.1.1.2.1.54.0"; # Total memory
my $foundry_free          = "1.3.6.1.4.1.1991.1.1.2.1.55.0"; # Free memory

# fortigate FW

my $forti_used          = "1.3.6.1.4.1.12356.101.4.1.4.0"; # Free memory
my $forti_total         = "1.3.6.1.4.1.12356.101.4.1.5.0"; # Total memory
my $forti_used_50       = "1.3.6.1.4.1.12356.1.9.0"; # Used memory
...
my $o_cisco_nexus=	undef;		# Check cisco nexus mem
my $o_foundry=	undef;		# Check foundry mem
my $o_forti=	undef;		# Check forti mem
my $o_forti_50=	undef;		# Check forti 50 mem
...
-n, --cisconexus
   check cisco nexus memory (used and free memory pools)
-F, --foundry
   check foundry lb memory (used and free memory pools)
-e, --forti
   check fortigate memory (used and free memory pools)
-i, --forti50
   check fortigate memory (used memory)
-E, --hp
...
sub check_options {
	...
	'n'	=> \$o_cisco_nexus,		'cisconexus'		=> \$o_cisco_nexus,
	'F'	=> \$o_foundry,		'foundry'		=> \$o_foundry,
	'e'	=> \$o_forti,		'forti'		=> \$o_forti,
	'i'	=> \$o_forti_50,		'forti50'	=> \$o_forti_50,
    ...
    if (defined($o_cisco) || defined($o_hp) || defined($o_cisco_nexus) || defined($o_foundry) || defined($o_forti) || defined($o_forti_50)) {
      $o_netsnmp=undef;
...
if (defined ($o_cisco)) {
  ...
  my $c_status="OK";
  $c_output .="Free Memory : " . $prct_free ."% ";
  if ($crit_s == 1 ) {
    $c_output .= " > " . $o_crit ;
    $c_status="CRITICAL";
  } else {
...
########### Cisco nexus memory check ############
if (defined ($o_cisco_nexus)) {

  # Test every memory pool
  my ($c_output,$prct_used)=(undef,undef);
  my ($warn_s,$crit_s)=(0,0);
  my ($used,$free)=(0,0);
  my (@get_used_free) = ();
  ...
  my $total=$used->{$cisco_nexus_used}+$free->{$cisco_nexus_free}; 
  $c_output = "";
  $prct_used=round($used->{$cisco_nexus_used}*100/($total),0);
  verb("Total used : $used->{$cisco_nexus_used}, free: $free->{$cisco_nexus_free}, output : $c_output");
  my $c_status="OK";
  if (($o_crit!=0)&&($o_crit <= $prct_used)) {
    $crit_s =1;
  } elsif (($o_warn!=0)&&($o_warn <= $prct_used)) {
    $warn_s=1;
  } else {
    $crit_s =0;
  }

  $c_output .="Used Memory : " . $prct_used ."%";
  if ($crit_s == 1 ) {
    $c_output .= " > " . $o_crit . "%" ;
    $c_status="CRITICAL";
  } else {
  ...
  $c_output .= " ; ".$c_status;
  if (defined ($o_perf)) {
    $c_output .= " | memory_used=" . $used->{$cisco_nexus_used}.";";
    $c_output .= ($o_warn ==0)? ";" : round($o_warn * $total/100,0).";"; 
    $c_output .= ($o_crit ==0)? ";" : round($o_crit * $total/100,0).";";
    $c_output .= "0;" . $total ;
  }             
...
########### foudry LB memory check ############
if (defined ($o_foundry)) {
  ...
  $used=$total->{$foundry_total}-$free->{$foundry_free};
  $c_output = "";
  $prct_used=round(100/$total->{$foundry_total}*$free->{$foundry_free},0);
  verb("Total used : $used, free: $free->{$foundry_free}, output : $c_output");
  my $c_status="OK";
  if (($o_crit!=0)&&($o_crit <= $prct_used)) {
    $crit_s =1;
  } elsif (($o_warn!=0)&&($o_warn <= $prct_used)) {
  ...
  $c_output .= " ; ".$c_status;
  if (defined ($o_perf)) {
    $c_output .= " | memory_used=" . $used.";";
    $c_output .= ($o_warn ==0)? ";" : round($o_warn * $total->{$foundry_total}/100,0).";";
    $c_output .= ($o_crit ==0)? ";" : round($o_crit * $total->{$foundry_total}/100,0).";";
    $c_output .= "0;" . $total->{$foundry_total} ;
  }
  $session->close;
  print "$c_output \n";
  exit $ERRORS{$c_status};
...
if (defined ($o_forti)) {
  ...
  $c_output = "";
  $used=round($total->{$forti_total}/100*$free->{$forti_used},0);
  verb("Total used : $used, free: $free->{$forti_used}, output : $c_output");
  my $c_status="OK";
  if (($o_crit!=0)&&($o_crit <= $free->{$forti_used})) {
    $crit_s =1;
  } elsif (($o_warn!=0)&&($o_warn <= $free->{$forti_used})) {
    $warn_s=1;
  } else {
    $crit_s =0;
  }

  $c_output .="Used Memory : " . $free->{$forti_used} ."%";
  if ($crit_s == 1 ) {
    $c_output .= " > " . $o_crit . "%" ;
    $c_status="CRITICAL";
  } else {
  ...
  if (defined ($o_perf)) {
    $c_output .= " | memory_used=" . $used.";";
    $c_output .= ($o_warn ==0)? ";" : round($o_warn * $total->{$forti_total}/100,0).";";
    $c_output .= ($o_crit ==0)? ";" : round($o_crit * $total->{$forti_total}/100,0).";";
    $c_output .= "0;" . $total->{$forti_total} ;
  }
  $session->close;
  print "$c_output \n";
  exit $ERRORS{$c_status};
...
########### Fortigate 50 FW memory check ############
if (defined ($o_forti_50)) {
  ...
  $c_output = "";
  $free=round(100 - $used->{$forti_used_50},0);
  verb("Total used : $used->$forti_used_50, free: $free, output : $c_output");
  my $c_status="OK";
  if (($o_crit!=0)&&($o_crit <= $used->{$forti_used_50})) {
    $crit_s =1;
  } elsif (($o_warn!=0)&&($o_warn <= $used->{$forti_used_50})) {
    $warn_s=1;
  } else {
    $crit_s =0;
  }

  $c_output .="Used Memory : " . $used->{$forti_used_50} ."%";
  if ($crit_s == 1 ) {
    $c_output .= " > " . $o_crit . "%" ;
    $c_status="CRITICAL";
  } else {
  ...
  $c_output .= " ; ".$c_status;
  if (defined ($o_perf)) {
    $c_output .= " | memory_used=" . $used->{$forti_used_50}.";";
    $c_output .= ($o_warn ==0)? ";" : round($o_warn,0).";";
    $c_output .= ($o_crit ==0)? ";" : round($o_crit,0).";";
    $c_output .= "0;";
  }
  $session->close;
  print "$c_output \n";
