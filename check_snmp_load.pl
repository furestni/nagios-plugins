...
# Cisco Nexus cpu/load

my $cisconexus_cpu_5m = ".1.3.6.1.4.1.9.9.109.1.1.1.1.8.1"; # Cisco CPU load (5min %)
my $cisconexus_cpu_1m = ".1.3.6.1.4.1.9.9.109.1.1.1.1.7.1"; # Cisco CPU load (1min %)
my $cisconexus_cpu_5s = ".1.3.6.1.4.1.9.9.109.1.1.1.1.6.1"; # Cisco CPU load (5sec %)

# Foundry LB cpu/load

my $foundry_cpu_1m = "1.3.6.1.4.1.1991.1.1.2.1.52.0"; # Foundry CPU load (1min %)
my $foundry_cpu_5s = "1.3.6.1.4.1.1991.1.1.2.1.51.0"; # Foundry CPU load (5sec %)
my $foundry_cpu_1s = "1.3.6.1.4.1.1991.1.1.2.1.50.0"; # Foundry CPU load (1sec %)
...
# Fortigate CPU
my $fortigate_cpu_50 = ".1.3.6.1.4.1.12356.1.8.0"; # Fortigate CPU % usage
my $fortigate_cpu = ".1.3.6.1.4.1.12356.101.4.1.3.0"; # Fortigate CPU % usage
...
# valid values 
my @valid_types = ("stand","netsc","netsl","as400","cisco","cata","nexus","foundry","nsc","fg","fg50","bc","nokia","hp","lp","hpux");
# CPU OID array
my %cpu_oid = ("netsc",$ns_cpu_idle,"as400",$as400_cpu,"bc",$bluecoat_cpu,"nokia",$nokia_cpu,"hp",$procurve_cpu,"lp",$linkproof_cpu,"fg",$fortigate_cpu,"fg50",$fortigate_cpu_50);
...
# check type  : stand | netsc |  netsl | as400 | cisco | cata | nexus | foundry | nsc | fg | fg50 | bc | nokia | hp | lp  | hpux
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>])  [-p <port>] -w <warn level> -c <crit level> -T=[stand|netsl|netsc|as400|cisco|cata|nexus|foundry|nsc|fg|fg50|bc|nokia|hp|lp|hpux] [-f] [-t <timeout>] [-V]\n";
}
...
-c, --crit=INTEGER | INT,INT,INT
   ...
   3 value check : comma separated level for load or cpu for 1min, 5min, 15min 
-T, --type=stand|netsl|netsc|as400|cisco|cata|nexus|foundry|bc|nokia|hp|lp
	CPU check : 
		...
		nexus : Cisco Nexus CPU usage
		foundry : Foundry Iron LB CPU usage
		nsc   : NetScreen CPU usage
		fg    : Fortigate CPU usage
		fg50  : Fortigate 50B CPU usage
...
sub check_options {
	...
	if (($o_check_type eq "netsl") || ($o_check_type eq "cisco") || ($o_check_type eq "cata") || ($o_check_type eq "nexus") ||
		($o_check_type eq "foundry") || ($o_check_type eq "nsc") || ($o_check_type eq "hpux")) {
...
if (!defined ($$resultat{$cisconexus_cpu_5s})) {
  print "No CPU information : UNKNOWN\n";
  exit $ERRORS{"UNKNOWN"};
}
...
$load[0]=$$resultat{$cisconexus_cpu_5s};
$load[1]=$$resultat{$cisconexus_cpu_1m};
$load[2]=$$resultat{$cisconexus_cpu_5m};
...
if (!defined ($$resultat{$foundry_cpu_1s})) {
  print "No CPU information : UNKNOWN\n";
  exit $ERRORS{"UNKNOWN"};
}
...
$load[0]=$$resultat{$foundry_cpu_1s};
$load[1]=$$resultat{$foundry_cpu_5s};
$load[2]=$$resultat{$foundry_cpu_1m};
...
if ( $o_check_type =~ /netsc|as400|bc|nokia|^hp$|lp|fg|fg50/ ) {
