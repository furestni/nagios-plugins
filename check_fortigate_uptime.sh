...
snmpget -v1 -c $community $host $oid | grep day > /dev/null
RETVAL=$?

output=$(snmpget -v1 -c $community $host $oid)
...
if [ $RETVAL -eq 0 ]
  ...
  echo "OK Uptime > 1 day:  $days day(s) $hours hour(s) and $minutes minute(s) | days=$days"
  exit 0
else
  days=0
  hours=`echo $output | awk '{print $5}' | awk -F ':' '{print $1}' | sed 's/^0//'`
  minutes=`echo $output | awk '{print $5}' | awk -F ':' '{print $2}' | sed 's/^0//'`
  echo "Warning Uptime < 1 day: Uptime $days day(s) $hours hour(s) and $minutes minute(s) | days=$days"
  exit 1
fi
